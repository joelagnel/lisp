...
;; Version: $Id: puzzle.el,v 1.15 2004/10/18 10:34:50 matt Exp $
...
(defun puzzle ()
  ...
  (puzzle-mode)
  (puzzle-restart)
  (set-window-buffer (selected-window) "*puzzle*"))
...
(defun puzzle-check-status ()
  "Check required features and some puzzle variables."
  (cond
   ((featurep 'xemacs)
...
(defun puzzle-header-line-format ()
  ...
  (let* ((string
         ...
         (length (floor (/ (- (min puzzle-image-char-width (frame-width))
                              (length string)) 2.0))))
