...
;; $Id: imath.el,v 1.4 2006/10/09 01:37:11 yasube Exp $
  ...
  (save-excursion
    (let* ((original-buffer (current-buffer))
	   ...
	   (filename (concat (file-name-sans-extension
			     ...
			     ".html"))
	   (image-folder (concat (file-name-sans-extension
				  (file-name-nondirectory current-buffer-file-name))
				 "-images"))
	   (html-buffer (find-file filename)))
      ;; create image folder
      (let (old-files)
	(if (listp (condition-case err
		       ;; list is returned if image-folder already exists.
		       (setq old-files (directory-files image-folder nil "^[0-9]+$"))
		     (file-error t))) ;; t is returned when directory does not exist.
...
(defun* imath-to-html()
  ...
  (if (not imath-mode)
      (progn
	(message "Imath minor mode is disabled in this buffer.")
