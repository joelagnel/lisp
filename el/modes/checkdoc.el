...
;; X-RCS: $Id: checkdoc.el,v 1.87 1998/06/15 23:17:24 zappo Exp $
...
;; entry points to all of different checks that are available.  It
...
;;       Check for comments standards, including headings like Code:
...
;; 0.3   Regexp changes for accuracy on var checking and param checking.
...
;;         first line a complete sentence.
...
;;         being parameters that need not show up in the doc string.
...
;;       Added `checkdoc-triple-semi-comment-check-flag'.
...
;; 0.6   Many cosmetic changes from RMS.
;;       Added check to capitalize proper nouns common in Emacs Lisp..
;;       Added new qualifier `checkdoc-in-example-string-p' so that
;;         symbols and words appearing in strings are correctly skipped.
;;       Added variable `checkdoc-symbol-words' which are words that are also
;;         symbols which we don't want quoted.
;;       An inserted footer now has an additional space between lines.
;;       Renamed `checkdoc' to `checkdoc-start' and created a new `checkdoc'
;;         which calls every check in an interactive fashion.
;;       Fixed an overlay bug in `checkdoc' (which was `checkdoc-interactive')
;;       Made `checkdoc' much faster and easier to use.
;;       Changed order of some tests to prevent unwanted combinations.
;;       Added a check for two spaces after a period.
;;       Michael Ernst <mernst@alum.mit.edu>
;;         Various punctuation and spelling errors fixed.
;;         Fixed `checkdoc-message-text-engine' to also find multiple spaces
;;           after the y-or-n-p questions.
;;         Addition of `checkdoc-permit-comma-termination-flag' and tiny check.
;;         Do not remove rogue white space after comment chars.
;;       Fixed `checkdoc-autofix-ask-replace' to not ask questions for
;;         complex replaces iff the autofix-flag is nil or never.
;;       Added `checkdoc-message-interactive' function and turned the
;;         old `checkdoc-interactive' into a function that takes a
;;         function that will find/fix a class of errors.
;;       New auto-fix for renaming variables to have `-flag' on the end.
;; 0.6.1 Revamped the error returning code.  Errors are now cached into
;;         the warning buffer when they are found, and the specific position
;;         is used.  When `-interactive' functions are used, the exact
;;         problem region is highlighted instead of the whole buffer.
;;       Fixed problem warning to add non-existant doc-strings.
;;       Fixed note-taking for `y-or-n-p' error checking section.
;;       Output to `checkdoc-diagnostic-buffer' now consists of one header
;;         per interactivly called check.  The header has much less info in it,
;;         and the file name is repeated on each line.
;;       Michael Ernst <mernst@alum.mit.edu>
;;         Disambiguated some error messages.
;;         Fixed some recentering issues.
;;         Added variable `checkdoc-force-history-flag'.
;;         Enable checkdoc-params code-customization to not require the
;;           parameters to be in a list.
;;       Strengthened reader of checkdoc-params to handle more obtuse cases.
;;       Replaced use of `y-or-n-p' when autofixing to use new function
;;         `checkdoc-y-or-n-p' which checks `checkdoc-autofix-flag'
;;         for it's return value.
;;       New value for `checkdoc-autofix-flag' is `automatic-then-never'
;;         used by `checkdoc-interactive-loop' to fix one error, and have
;;         the next one returned.
;;       `checkdoc-recursive-edit' message now makes sense for
;;         non-documentation string situations.
;;       Added autofixes for missing Commentary and History sections.
...
(defvar checkdoc-symbol-words nil
  ...
  (let* ((begin (prog1 (point)
	...
	(unwind-protect
		...
		(let ((pe (car err-list))
		  ...
		  ;; Provide a small message saying nothing was available.
		  (if (and (equal ne pe)
			   (or (checkdoc-char= last-input-char ?f)
			       (checkdoc-char= last-input-char ?y)))
		      (progn
...
function,command,variable,option or symbol." (match-string 3)))))))
	 (if ret
	     (checkdoc-create-error ret (match-beginning 3)
				    (match-end 3))
	   nil)))
...
Code:, and others referenced in the style guide."
  ...
  (save-excursion
	...
	(if (or checkdoc-force-history-flag
