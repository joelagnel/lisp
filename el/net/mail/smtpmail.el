	  ...
	  (let ((cred (smtpmail-find-credentials smtpmail-starttls-credentials host port)))
		...
		(let ((starttls-extra-args
		       (when (and (string-to-list (smtpmail-cred-key cred))
				  ...
				  (file-regular-p (expand-file-name (smtpmail-cred-key cred)))
				  (file-regular-p (expand-file-name (smtpmail-cred-cert cred))))
			 (list "--key-file" (expand-file-name (smtpmail-cred-key cred))
			       "--cert-file" (expand-file-name (smtpmail-cred-cert cred))))))
	  ...
	  (save-excursion
		...
		(cond ((eq mech 'cram-md5)
			 ...
			 (let* ((challenge (substring (cadr response-code) 4))
				...
				(hash (rfc2104-hash 'md5 64 16 (or (smtpmail-cred-passwd cred) passwd) decoded))
		      ...
		      ((eq mech 'login)
		       ...
		       (if (or (null (car (setq response-code (smtpmail-read-response process))))
			       ...
			       (>= (car response-code) 400))
			   (throw 'done nil))
		       (smtpmail-send-command process (base64-encode-string (smtpmail-cred-user cred)))
		       (if (or (null (car (setq response-code (smtpmail-read-response process))))
			       (not (integerp (car response-code)))
			       (>= (car response-code) 400))
			   (throw 'done nil))
		       (smtpmail-send-command process (base64-encode-string (or (smtpmail-cred-passwd cred) passwd)))
		       (if (or (null (car (setq response-code (smtpmail-read-response process))))
			       (not (integerp (car response-code)))
			       (>= (car response-code) 400))
			   (throw 'done nil)))
