...
;;; ;;; Subject: wordnet.el v0.2
;;; ;;; Newsgroups: gnu.emacs.sources
;;; ;;; Date: Thu, 21 Nov 2002 12:02:30 +0100
;;; ;;; Organization: Vienna University, Austria

;;; A simple mode for locally installed dictionaries -- like wordnet's wn,
;;; and including dumped output (e.g. via w3m) of web dictionaries.
...
;;; This requires tellib -- posted here earlier today.
...
;; *** Important Commands
;; 
;; wordnet-lookup :: Lookup the word at point.
;; 
;; wordnet-lookup-specific :: Lookup the word at point in a specific
;; dictionary.
;; 
;; wordnet-run-wn-with-word-at-mouse :: Lookup the word the mouse is
;; pointing at.
;; 
;; 
;; *** Important Variables
...
(defun wordnet-get-command-line (&optional mode)
  ...
  (if mode
	...
	(when rv
	  (list rv)))
    (let (rv)
      (dolist (this wordnet-command-line rv)
	(when (wordnet-get-command-line-option this :overview)
	  (add-to-list 'rv this t))))))
...
(defun wordnet-lookup (&optional dont-ask mode volatile)
  ...
  (let ((word (wordnet-thing-to-lookup))
    ...
    (wordnet-display-result (wordnet-run-wn word dont-ask mode)
			    mode
			    volatile)))
(fset 'wordnet-run-wn-with-word-at-point 'wordnet-lookup)
...
(defun wordnet-install ()
  "Install wordnet hooks."
  (tellib-installer 'tellib 'wordnet))

(defun wordnet-uninstall ()
  "Deinstall wordnet hooks."
  (tellib-uninstaller 'tellib 'wordnet))
