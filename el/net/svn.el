...
;;; ;;; From: Stefan Reichör <reichoer@web.de>
;;; ;;; Subject: svn.el - an emacs interface for subversion
;;; ;;; Newsgroups: gnu.emacs.sources
;;; ;;; Date: Tue, 20 Aug 2002 08:08:10 +0200
;;; ;;; Organization: Johannes Kepler Universitaet Linz

;;; --=-=-=

;;; I have implemented an emacs interface for subversion.
...
;;; Content-Disposition: attachment; filename=svn.el
;;; Content-Transfer-Encoding: 8bit
;;; Content-Description: svn.el

;;; svn.el --- Subversion interface for emacs
...
;; $LastChangedDate: 2002-08-11 21:28:58 +0200 (So, 11 Aug 2002) $



;; svn.el is free software; you can redistribute it and/or modify
...
;; svn.el is distributed in the hope that it will be useful,
...
;; svn.el is tested with GNU Emacs 21.1.1 on windows with svn 0.14.0



;; svn.el is an interface for the revision control tool subversion

;; (see http://subversion.tigris.org)

;; svn.el provides a similar interface for subversion as pcl-cvs for cvs.
...
;; To use svn.el put the following line in your .emacs:
...
;; The latest version of svn.el can be found at:

;;   http://xsteve.nit.at/prg/emacs/svn.el



;; TODO:

;; shortcut for svn propset svn:keywords "Date" svn.el
...
(defvar svn-status-temp-dir temporary-file-directory)
...
(defun svn-parse-status-result ()
  ...
  (save-excursion
    ...
    (let ((old-marked-files (svn-status-marked-file-names))
      ...
      (while (> (- (line-end-position) (line-beginning-position)) 0)
        ...
        (if (string-match "Head revision:[ ]+\\([0-9]+\\)" line-string)
          ...
          (setq file-svn-info (split-string line-string))
...
(defun svn-status-line-info->hide-because-unmodified (line-info)
  ...
  ;;(message " %S %S" svn-status-hide-unmodified (svn-status-line-info->propmark line-info))
...
(defun svn-status-next-line (nr-of-lines)
  ...
  (goto-char (+ (line-beginning-position) svn-status-default-column)))
...
(defun svn-status-apply-usermark (set-mark only-this-line)
  ...
  (let* ((st-info svn-status-info)
    ...
    (svn-status-goto-file-name file-name)))
