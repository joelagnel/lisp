...
`ddl-column-marker-1' & `ddl-column-marker-2'"
  :group 'ddl
  :group 'faces)
...
Use `C-u <column> \\[ddl-column-marker-1]' while in a ddl-mode
buffer to change the column marker interactively in that buffer
only.  This customize option sets the default for ddl-mode."
  :type 'integer
  :group 'ddl)
(make-variable-buffer-local 'ddl-column-marker-1)
(defun ddl-column-marker-1 (column)
  "Set the column marker to COLUMN for the current `ddl-mode' buffer.
Max value allowed is 132.  0 turns off the marker.  Set the default with
`M-x customize-option <ret> `ddl-column-marker-1' <ret>'. For this buffer
only, specify the column with `C-u <column> \\[ddl-column-marker-1]' or
`M-x ddl-column-marker-1 RET <column> RET'."
  (interactive "NMarker Column: ")
  (if (not(equal major-mode 'ddl-mode))
      (error "Current buffer must be ddl-mode")
    (if (< column 0)(setq column 0))
    (if (> column 132)(setq column 132))
    (setq ddl-column-marker-1 column)
...
particular column has significance.  Use `\\[ddl-column-marker-2]' RET num
while in a ddl-mode buffer to change the column marker interactively in
that buffer only.  This customize option sets the default for ddl-mode."
  :type 'integer
  :group 'ddl)
(make-variable-buffer-local 'ddl-column-marker-2)
(defun ddl-column-marker-2 (column)
  "Set the column marker to COLUMN for the current `ddl-mode' buffer.
Max value allowed is 132.  0 turns off the marker.  Set the default with
`M-x customize-option <ret> `ddl-column-marker-2' <ret>'. For this buffer
only, specify the column with `C-u <column> \\[ddl-column-marker-2]' or
`M-x ddl-column-marker-2 RET <column> RET'."
  (interactive "NMarker Column: ")
  (if (not(equal major-mode 'ddl-mode))
      (error "Current buffer must be ddl-mode")
    (if (< column 0)(setq column 0))
    (if (> column 132)(setq column 132))
    (setq ddl-column-marker-2 column)
    ;; now to make it take effect immediately I turn off font-lock, make
    ;; font-lock think it has not been on in this buffer previously, then
    ;; re-do our normal font-lock initialization which typically turns it
    ;; back on.  Only this time the new config is in place.
    ...
    (setq font-lock-set-defaults nil) ;key to it not ignoring my changes
    (ddl-setup-font-lock)))
(defun ddl-column-marker-string ()
  (let ((A (min ddl-column-marker-1 ddl-column-marker-2))
        (B (max ddl-column-marker-1 ddl-column-marker-2)))
    (setq A (if (zerop A) ""
...
(defun ddl-find-syntactic-keywords ( search-limit )
  ...
  ;; !  go until another ! or eol.  If column 1 contains * thats a comment
  ;; until eol too.  This fcn returns nil if no comment is found.  It returns
  ;; t and match-data 1&2 indicating the strings to get `<' and `> syntax'.
...
(defun ddl-build-font-lock-keywords ()
  ...
  (append ddl-static-font-lock-keywords
          `(,(unless (and (zerop ddl-column-marker-1)
                       (zerop ddl-column-marker-2))
               (list (ddl-column-marker-string)
                     '(1 ddl-column-marker-face prepend t)
                     '(2 ddl-column-marker-face prepend t)))
...
See the customization options for ddl-mode!  They are available via:
...
It also implements the following M-x ... commands

`ddl-mode'	       Activates this mode for the current buffer
`ddl-column-ruler'     Temporarily displays column ruler in buffer
`ddl-column-marker-1'  \These move the column markers for this
`ddl-column-marker-2'  /buffer only. See C-h f ddl-column-marker-1
