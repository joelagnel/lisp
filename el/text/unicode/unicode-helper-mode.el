...
;; URL: http://www.emacswiki.org/elisp/unicode-helper.el
...
(defun unicode-helper-name-at-point ()
  ...
  (let* ((pos (point))
         (char (char-after pos))
         (unicode (when (or (< char 256)
                            (memq 'mule-utf-8
                                  (find-coding-systems-region pos (1+ pos)))
                            (get-char-property pos 'untranslated-utf-8))
                    (or (get-char-property pos 'untranslated-utf-8)
                        (encode-char char 'ucs))))
         (codepoint (when unicode
                      (format "%04X" unicode))))
    (if (not codepoint)
        (message "Not a Unicode character")
