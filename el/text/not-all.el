...
;;; 96.01.10 modified by Yamaoka
  (define-key all-mode-map "\C-c\C-c" 'all-mode-goto)
  (define-key all-mode-map [down-mouse-2] 'undefined)
  (define-key all-mode-map [mouse-2] 'all-mode-goto-by-mouse))
;;;
...
;;; 96.01.10 modified by Yamaoka
(defun all-mode-goto-by-mouse (event)
  (interactive "e")
...
;;; 96.01.10 modified by Yamaoka
	(if (eq window-system 'x)
	    (insert "Click mouse-2 to go to the original buffer.\n")
	  (insert "Type \"C-c C-c\" to go to the original buffer.\n"))
;;;
	(insert "--------\n"))
...
(defun all-insert ()
  ...
  (save-excursion
    ...
    (let ((from (point))
;;; 96.01.10 modified by Yamaoka
	  to buffer-read-only ovl)
;;;
      (insert-buffer-substring buffer prevstart prevend)
      (setq to (point))
;;; 96.01.10 modified by Yamaoka
      (setq ovl (make-overlay from to))
      (overlay-put ovl 'marker marker)
