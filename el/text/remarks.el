...
;;; ;;; Subject: remarks.el v0.2
;;; ;;; Newsgroups: gnu.emacs.sources
;;; ;;; Date: 21 Nov 2002 10:07:55 +0100
;;; ;;; Organization: Vienna University, Austria

;;; Add remarks/annotations to text in a non-intrusive way -- i.e. the
;;; remarks are stored seperately. It turns out that remarks can also be
;;; links to other files.

;;; This requires some other libraries, I will post here later.
...
;; nevertheless force saving notes by calling `properties-write'.
;; 
...
-- You can force saving notes by calling `properties-write'.
"
  (interactive)
  (properties-add (or counter-var 'remarks-counter))
  (properties-overlay-add 'remarks))
...
"
  (interactive "r")
  (tellib-with-key-arguments
   args ((:class            class 'remarks)
	 (:use-counter-var  this-counter)
	 (:use-marker-var   this-marker)
	 (:use-face         this-face)
	 (:use-edit-mode    this-mode)
	 (:use-edit-map     this-map)
	 (:use-write-buffer this-write-buffer)
	 (:use-read-buffer  this-read-buffer)
	 (:use-emptyp       this-emptyp)
	 (:use-edit-remark  this-edit-function)
	 (:text             text "")
	 (:text!            text!)
	 (:message          message "Make the first line a significant title.")
	 (:dont-edit        dont-edit))
		...
		(when (not (funcall #',emptyp txt))
		  (save-excursion
			...
			(overlay-put ol 'before-string (format ,marker ,count)))
		      (when ,this-edit-function
			(overlay-put ol 'remarks-edit-remark #',this-edit-function))
...
PRETTYPRINT overrides the CLASS definition if provided."
  ...
  (let* ((ols (tellib-overlay-collect*
    ...
    (dolist (this (tellib-overlay-collect* '(remarks)))
      (delete-overlay this))
    (current-buffer)))


(defun remarks-install ()
  "Install remarks hooks."
  (tellib-installer '(tellib properties) 'remarks))

(defun remarks-uninstall ()
  "Deinstall remarks hooks."
  (tellib-uninstaller '(tellib properties) 'remarks))
